...
explanation of tfidf vectorizer
...
Finally, we will use `MultinomialNB` to train the tf-idf vectors with a Naive-Bayes classifier. This will be the final model that is used to classify tweet sentiment. ``` model = MultinomialNB().fit(messages_tfidf) ```
...
model performance for each estimator (by accuracy)
...
model performance on each estimator (by computational time)
